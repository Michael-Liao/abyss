...
void neg(const uint8_t* in_data, const size_t* ids, const size_t n,
         uint8_t* out_data) {
  for (size_t i = 0; i < n; i++) {
    out_data[i] = -in_data[ids[i]];
  }
}
void neg(const int32_t* in_data, const size_t* ids, const size_t n,
         int32_t* out_data) {
  for (size_t i = 0; i < n; i++) {
    out_data[i] = -in_data[ids[i]];
  }
}
void neg(const double* in_data, const size_t* ids, const size_t n,
         double* out_data) {
  for (size_t i = 0; i < n; i++) {
    out_data[i] = -in_data[ids[i]];
  }
